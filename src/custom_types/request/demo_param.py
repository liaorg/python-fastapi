...
class DemoParam(BaseModel):
    ...
    class Config:
        ...
        json_schema_extra = {
            "example": {"user_name": "张三", "age": 89, "city": "北京"}
        }
...
class PydanticVerifyParam(BaseModel):
    ...
    user_name: str  # 基本类型
    age: int = Field(ge=18, le=30)  # 整数范围：18 <= age <= 30
    password: int = Field(min_length=6, max_length=10)  # 字符长度
    phone: int = Field(pattern=r"^1\d{10}$")  # 正则验证手机号
    # phone: str  # 正则验证手机号
    ...
    items: List[
        Annotated[str, Field(min_length=1, max_length=3)]
    ]  # 限制列表中的每个元素长度的范围
    email: EmailStr  # 邮箱格式
...
class StudentParam(BaseModel):
    ...
    name: int = Field(min_length=2, max_length=4)  # 长度
    age: int = Field(ge=18, le=30)  # 整数范围：18 <= age <= 30
    class_name: str  # 班级名称
...
class FieldParam(BaseModel):
    ...
    age: int = Field(
        default="", gt=18, description="填写年龄,必须大于18", examples=[20]
    )
    phone: str = Field(
        default="",
        description="填写手机号",
        examples=["17600000000"],
        pattern=r"^1\d{10}$",
    )
    likes: List[str] = Field(
        description="填写爱好", examples=[["篮球", "足球"]], min_length=2
    )
